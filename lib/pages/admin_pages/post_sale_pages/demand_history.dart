...
class _DemandHistoryPageState extends State<DemandHistoryPage> {
  ...
  void _loadMockData() {
    ...
    Future.delayed(const Duration(seconds: 2), () {
      setState(() {
        demands = List.generate(20, (index) => Demand(
         
          demandpaidDate: '2023-${(index % 12 + 1).toString().padLeft(2, '0')}-${(index % 28 + 1).toString().padLeft(2, '0')}',
          // amount: (index + 1) * 100.0,
          amount: 74857,
          clientName: 'Client ${index + 1}',
          projectName: '9square', slab: '34', flatno: '1203', reminder: 'I',
        ));
        isLoading = false;
      });
    });
  ...
  List<Demand> getFilteredDemands() {
    ...
    return demands.where((demand) =>
      ...
      demand.clientName.toLowerCase().contains(searchQuery.toLowerCase()) ||
      demand.projectName.toLowerCase().contains(searchQuery.toLowerCase())
    ).toList();
  }
