...
class EditSiteVisitPage extends StatefulWidget {
  ...
  const EditSiteVisitPage({super.key, required this.visit});
...
class _EditSiteVisitPageState extends State<EditSiteVisitPage> {
  ...
  Future<void> onVerfiredOrSkipOtp([bool verified = false]) async {
    try {
      ...
      try {
        ...
        await settingProvider.updateSiteVisit(widget.visit!.id!, visitMap);
      } catch (e) {
  ...
  void initState() {
    ...
    _onRefresh();
    selectedVisit = widget.visit.visitType;
    selectedProj = widget.visit.location;
    _selectedDate = widget.visit.date;
    firstNameController.text = widget.visit.firstName ?? "";
    lastNameController.text = widget.visit.lastName ?? "";
    phoneController.text = widget.visit.phoneNumber?.toString() ?? "";
    emailController.text = widget.visit.email ?? "";
    addressController.text = widget.visit.residence ?? "";
    selectedProject = widget.visit.projects ?? [];
    selectedRequirement = widget.visit.choiceApt ?? [];
    // _selectedSource = widget.visit.source;
    _selectedSalesManagers1 = widget.visit.closingTeam;
    _selectedSeniorClosingManager = widget.visit.closingManager;
  }
