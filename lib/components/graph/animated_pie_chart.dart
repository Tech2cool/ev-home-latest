...
class _AnimatedPieChartState extends State<AnimatedPieChart> with SingleTickerProviderStateMixin {
  ...
  void didUpdateWidget(AnimatedPieChart oldWidget) {
    ...
    if (oldWidget.visited != widget.visited || oldWidget.notVisited != widget.notVisited) {
      _controller.reset();
      _controller.repeat();
    }
  }
...
class PieChartPainter extends CustomPainter {
  ...
  void _drawAnimatedParticles(Canvas canvas, Size size, double visitedAngle, double notVisitedAngle) {
    ...
    for (var i = 0; i < particleCount; i++) {
      ...
      final distanceFromCenter = radius * (0.6 + 0.2 * sin(baseAngle * 3 + animation * 10));
      ...
      if (baseAngle <= visitedAngle - pi / 2 || baseAngle > 2 * pi - pi / 2) {
        canvas.drawCircle(particlePosition, particleRadius, visitedParticlePaint);
      } else {
        canvas.drawCircle(particlePosition, particleRadius, notVisitedParticlePaint);
      }
    }
  }
