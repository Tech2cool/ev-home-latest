...
class EditNewProjectPage extends StatefulWidget {
  final OurProject ourProj;

  const EditNewProjectPage({
    super.key,
    required this.ourProj,
  });

  @override
  State<EditNewProjectPage> createState() => _EditNewProjectPageState();
}

class _EditNewProjectPageState extends State<EditNewProjectPage> {
  final _formKey = GlobalKey<FormState>();

  final bool _projectdetails = false;
  ...
  void showcaseImages(File? showcaseImage) {
    if (showcaseImage != null) {
      setState(() {
        _showCaseImage = showcaseImage;
      });
    }
  }

  void selectedBrochures(File? selectedBrochure) {
    if (selectedBrochure != null) {
      setState(() {
        _selectedBrochure = selectedBrochure;
      });
    }
  }
    ...
    try {
      if (_showCaseImage != null) {
        ...
        if (uShowcaseImage != null) {
          setState(() {
            showcaseImg = uShowcaseImage.downloadUrl;
            _showCaseImage = null;
          });
        }
      }
      ...
      if (_selectedBrochure != null) {
        ...
        if (uBrochurResp != null) {
          setState(() {
            brochure = uBrochurResp.downloadUrl;
      ...
      await settingProvider.editOurProject(widget.ourProj.id!, projectjson);
      if (mounted) {
        ...
        child: Stack(
          children: [
            Scaffold(
              ...
              appBar: AppBar(title: const Text('Edit New Project')),
                          ...
                          children: [
                                ...
                                if (showcaseImg != null)
                                  Row(
                                    children: [
                                      ...
                                      const SizedBox(width: 10),
                                      const Text("Selected showcase Image"),
                                      const Spacer(),
                                      IconButton(
                                        onPressed: () {
                                          setState(() {
                            ...
                            Column(
                              children: [
                                ...
                                const SizedBox(height: 15),
                                ...List.generate(carouselImg.length, (i) {
                                  final image = carouselImg[i];
                                  return Row(
                                    children: [
                                      SizedBox(
                                        width: 50,
                            ...
                            if (amenity.isNotEmpty) ...[
                              ...
                              const SizedBox(height: 10),
                              ...List.generate(amenity.length, (i) {
                                final amenityimage = amenity[i];
                                return Row(
                                  children: [
                                    SizedBox(
                                      width: 50,
                              ...
                              ...List.generate(config.length, (i) {
                                return Row(
                                  children: [
                                    ...
                                    const SizedBox(width: 10),
                                    Text(
                                      '${config[i].configuration} / ${config[i].carpetArea} / ${Helper.formatNumber(config[i].price)}',
                                    ),
                                    const Spacer(),
                                    IconButton(
                                      onPressed: () {
